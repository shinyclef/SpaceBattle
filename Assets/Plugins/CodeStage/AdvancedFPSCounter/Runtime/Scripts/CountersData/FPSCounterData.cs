namespace CodeStage.AdvancedFPSCounter.CountersData
{
	...
	public class FPSCounterData: UpdatableCounterData
	{
...
#if UNITY_EDITOR
		[SerializeField]
		private bool realtimeFPSFoldout;
#endif
		...
		{
			get { return realtimeFPS; }
			set
			{
				if (realtimeFPS == value || !Application.isPlaying) return;
				realtimeFPS = value;
				if (!realtimeFPS) LastValue = 0;
				if (!enabled) return;
		...
		internal override void UpdateValue(bool force)
		{
			...
			if (average)
			{
				...
				{
					currentAverageRaw += (LastValue - currentAverageRaw) / (currentAverageSamples + 1);
				}
